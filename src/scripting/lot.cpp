...
namespace BlueBear {
	namespace Scripting {
		...
		void Lot::buildFloorMap( Json::Value& floor ) {
			...
			// Create the vector reference of shared_ptrs by iterating through dict
			std::vector< std::shared_ptr< Models::Tile > > lookup;
			for( const Json::Value& dictEntry : dict ) {
			...
			floorMap = std::make_unique< Containers::Collection3D< std::shared_ptr< Models::Tile > > >( stories, floorX, floorY );
			...
			for( Json::Value& level : levels ) {
				for( Json::Value& object : level ) {
					if( Tools::Utility::isRLEObject( object ) ) {
						// De-RLE the object
						unsigned int run = object[ "run" ].asUInt();
						std::shared_ptr< Models::Tile > entry = getTile( object[ "value" ].asInt(), lookup );
		 ...
		 */
		std::shared_ptr< Models::Tile > Lot::getTile( int index, std::vector< std::shared_ptr< Models::Tile > >& lookup ) {
			std::shared_ptr< Models::Tile > entry;
